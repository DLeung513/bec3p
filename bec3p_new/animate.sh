...
norm=10001000
delay=20
# Scaling from kpc/T to km/s; T = 1.61e11 s
scale=191300
range=*
contour=0
...
(
	vr=""
	if [[ "$vrotfile" != "" ]] ; then
		vr="'${vrotfile}' using 1:2:(\$2-\$3):(\$2+\$4) with yerrorbars ps 0 lc rgb 'black' notitle,'' using 1:2 with points ls 5 lc rgb 'black' ps 1.25 notitle,"
	fi
	echo set terminal gif medium size 640,480
	echo set output \"/dev/null\"
	echo unset title
	...
	echo set yrange [0:*] writeback
	echo old_x=NaN
	echo old_y=NaN
	echo "plot 'grav${plane}${norm:1:7}.dat' using ((abs(\$2)<=$mincp&&abs(\$3)<=$mincp)?(dx=\$1-old_x,old_x=\$1,\$1-dx/2):1/0):((abs(\$2)<=$mincp&&abs(\$3)<=$mincp)?(dy=\$4-old_y,old_y=\$4,$scale*sqrt(abs(dy/dx*\$1))):1/0) w l notitle"
	echo set terminal gif medium size 640,480 animate delay ${delay} loop 0 optimize
	echo set output \"$outfile\"
	echo set yrange restore
	for i in {10000000..11500000..1} ; do
		if [ -f grav${plane}${i:1:7}.dat ] ; then
			...
			echo old_x=NaN
			echo old_y=NaN
			echo "plot ${vr}'grav${plane}${i:1:7}.dat' using ((abs(\$2)<=$mincp&&abs(\$3)<=$mincp)?(dx=\$1-old_x,old_x=\$1,\$1-dx/2):1/0):((abs(\$2)<=$mincp&&abs(\$3)<=$mincp)?(dy=\$4-old_y,old_y=\$4,$scale*sqrt(abs(dy/dx*\$1))):1/0) w l notitle"
		fi
	done
	echo 1>&2
) | gnuplot
